...
def run_smoke_test():
    print("=" * 70)
    print("⚡ HOLYEASYLANG CLEAN-ENVIRONMENT SMOKE TEST")
    print("=" * 70)

    temp_sandbox = tempfile.mkdtemp(prefix="he_clean_smoke_")
    ...
    try:
        # Step 1: he doctor
        print("[1/5] Testing 'he doctor' in fresh environment...")
        ...
        # Step 4: he test
        print("[4/5] Testing 'he test' for in-file unit tests...")
        res = subprocess.run([sys.executable, cli_py, "test", main_he], cwd=proj_dir, capture_output=True, encoding='utf-8', errors='replace')
        ...
        if "Hello, smoke_project from HolyEasyLang!" not in res_py.stdout:
            print("PY STDOUT:", repr(res_py.stdout))
            print("PY STDERR:", repr(res_py.stderr))
