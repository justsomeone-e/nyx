...
class UniversalCodeGen:
    ...
    def gen_react(self) -> str:
        lines = [
            "// Auto-generated by nyx Compiler (hereact - React 19 TSX)",
        ...
        for s in self.ast.statements:
            if isinstance(s, VarDeclNode):
                val_s = json.dumps(s.expr.value) if isinstance(s.expr, (NumberNode, StringNode, BooleanNode)) else "null"
                ...
                for bs in s.body:
                    if isinstance(bs, ReturnNode):
                        r_val = bs.expr.name if isinstance(bs.expr, IdentifierNode) else str(bs.expr.value) if hasattr(bs.expr, 'value') else "null"
                        lines.append(f"    return {r_val};")
                lines.append("  };")
        ...
        lines.extend([
            "",
            "  return (",
            "    <div style={{ padding: 32, background: '#05070a', color: '#00f0ff', fontFamily: 'JetBrains Mono, monospace', minHeight: '100vh' }}>",
            "      <h1 style={{ borderBottom: '1px solid #1e293b', paddingBottom: 12 }}>⚡ nyx Reactive UI Component</h1>",
            "      <div style={{ background: '#0c131d', padding: 20, borderRadius: 8, border: '1px solid #1e293b', marginTop: 16 }}>",
            "        <h3 style={{ color: '#94a3b8', margin: '0 0 12px 0' }}>Live Output:</h3>",
            "        {logs.map((log, i) => (",
            "          <div key={i} style={{ color: '#4ade80', marginBottom: 4 }}>&gt; {log}</div>",
            "        ))}",
            "      </div>",
            "    </div>",
            "  );",
