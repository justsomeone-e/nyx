...
class BundleEmitter:
    ...
    def _get_module_ir(self) -> ModuleIR:
        if self._module_ir is None:
            self._module_ir = BundleLowerer(self._get_hir(), self.module_name).lower()
    ...
    def emit_mjs(self) -> str:
        return render_mjs(self.get_exported_functions(), self.module_name)
    ...
    def emit_react(self) -> str:
        lines = [
            ...
            f"import {{ initNyxModule, type NyxModule, type NyxModuleSource }} from './{self.module_name}.mjs';",
            f"export * from './{self.module_name}.mjs';",
            "",
            f"const defaultWasmSource = new URL('./{self.module_name}.wasm', import.meta.url);",
            "const modulePromiseCache = new Map<NyxModuleSource, Promise<NyxModule>>();",
            "",
            "function getModulePromise(wasmSource: NyxModuleSource = defaultWasmSource): Promise<NyxModule> {",
            "  if (!modulePromiseCache.has(wasmSource)) {",
            "    modulePromiseCache.set(wasmSource, initNyxModule(wasmSource));",
            "  }",
            "  return modulePromiseCache.get(wasmSource)!;",
            "}",
            "",
            "export function useNyxModule(wasmSource: NyxModuleSource = defaultWasmSource): NyxModule {",
    ...
    def emit_react_mjs(self) -> str:
        return "\n".join([
            f"// Auto-generated by nyx bundle ({self.module_name}.react.mjs)",
            "import { use } from 'react';",
            f"import {{ initNyxModule }} from './{self.module_name}.mjs';",
            f"const defaultWasmSource = new URL('./{self.module_name}.wasm', import.meta.url);",
            ...
            "  return modulePromiseCache.get(wasmSource);",
            "}",
            "export function useNyxModule(wasmSource = defaultWasmSource) {",
            "  return use(getModulePromise(wasmSource));",
            "}",
            f"export * from './{self.module_name}.mjs';",
            "",
    ...
    def emit_react_dts(self) -> str:
        return "\n".join([
            "import type { NyxModule, NyxModuleSource } from './" + self.module_name + ".mjs';",
            "export * from './" + self.module_name + ".mjs';",
            "export function useNyxModule(wasmSource?: NyxModuleSource): NyxModule;",
            "",
        ])

    def emit_vue_mjs(self) -> str:
        return "\n".join([
            f"// Auto-generated by nyx bundle ({self.module_name}.vue.mjs)",
            ...
            "  const ready = initNyxModule(wasmSource);",
    ...
    def emit_vue_dts(self) -> str:
        return "\n".join([
            ...
            f"import type {{ NyxModule, NyxModuleSource }} from './{self.module_name}.mjs';",
            f"export * from './{self.module_name}.mjs';",
            "export interface NyxVueState { module: DeepReadonly<Ref<NyxModule | null>>; error: DeepReadonly<Ref<unknown>>; loading: DeepReadonly<Ref<boolean>>; ready: Promise<NyxModule>; }",
            "export function useNyxModule(wasmSource?: NyxModuleSource): NyxVueState;",
            "",
        ])

    def emit_svelte_mjs(self) -> str:
        return "\n".join([
            f"// Auto-generated by nyx bundle ({self.module_name}.svelte.mjs)",
            "import { readable } from 'svelte/store';",
            f"import {{ initNyxModule }} from './{self.module_name}.mjs';",
            f"const defaultWasmSource = new URL('./{self.module_name}.wasm', import.meta.url);",
            "const stores = new Map();",
            "export function nyxModule(wasmSource = defaultWasmSource) {",
            ...
            "    initNyxModule(wasmSource).then(module => set({ module, error: null, loading: false })).catch(error => set({ module: null, error, loading: false }));",
            "    return () => {};",
            "  }));",
    ...
    def emit_svelte_dts(self) -> str:
        return "\n".join([
            ...
            f"import type {{ NyxModule, NyxModuleSource }} from './{self.module_name}.mjs';",
            f"export * from './{self.module_name}.mjs';",
            "export interface NyxSvelteState { module: NyxModule | null; error: unknown; loading: boolean; }",
            "export function nyxModule(wasmSource?: NyxModuleSource): Readable<NyxSvelteState>;",
            "",
        ])
