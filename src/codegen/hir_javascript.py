...
_JAVASCRIPT_RUNTIME = r'''// Auto-generated by Nyx Compiler (hejs HIR backend)
    ...
    def _emit_declaration(self, node: IRNode) -> List[str]:
        ...
        if isinstance(node, IRExternFunction):
            ...
            return [
                ...
                f"    throw new Error({self._string(f'extern {node.abi} function {node.name} is unavailable on hejs')});",
                "}",
            ]
        ...
        for case in node.cases:
            ...
            if isinstance(pattern, IRCall) and pattern.callee in ("Ok", "Err"):
                ...
                raise JavaScriptEmissionError(
                    f"Unsupported constructor pattern '{pattern.callee}' on hejs"
                )
            else:
